...
var ExperienciaEngine = (function () {
  ...
  function pointInRoundRect(px, py, x, y, w, h, rx) {
    ...
    else if (px > x + w - rx && py > y + h - rx) { cx = x + w - rx; cy = y + h - rx; }
    else { cx = x + rx; cy = y + h - rx; }
    var dx = px - cx;
    var dy = py - cy;
  ...
  /** Point-in-silhouette in SVG viewBox units — mirrors sceneShapeGeometry paint. */
  function shapeSilhouetteHitTestViewBox(kind, vx, vy, paint) {
    ...
    if (kind === 'SHAPE_LINE') {
      if (paint.contentW > 0 && paint.contentH > 0) {
        ...
        var lmidY = lch / 2;
        return distToSegmentSq(vx, vy, 0, lmidY, lcw, lmidY) <= strokeTol * strokeTol;
      }
      var lx1 = 50 - w / 2;
      var lx2 = 50 + w / 2;
    ...
    if (kind === 'SHAPE_CAPSULE') {
      if (paint.contentW > 0 && paint.contentH > 0) {
        ...
        return pointInRoundRect(vx, vy, 0, 0, capCw, capCh, capRx);
      }
      var capRxLegacy = u(8);
      var rxCap = Math.min(capRxLegacy, w / 2, h / 2);
      return pointInRoundRect(vx, vy, 50 - w / 2, 50 - h / 2, w, h, rxCap);
    }
    if (kind === 'SHAPE_ROUND_RECT') {
      if (paint.contentW > 0 && paint.contentH > 0) {
      ...
      return pointInRoundRect(vx, vy, 50 - w / 2, 50 - h / 2, w, h, rr);
    }
    if (paint.contentW > 0 && paint.contentH > 0) {
  ...
  function resolveShapeSilhouetteHitViewBox(kind, paint) {
    ...
    var st = shapeStretchXY(paint);
    var tightBb = shapeContentBBox(kind, st);
    return {
  ...
  /** Selection hit-test — silhouette only, never the gizmo bounding box. */
