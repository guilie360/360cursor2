...
var ExperienciaCanvas = (function () {
  ...
  /** Temporary — ?multiRotateDebug=1 for group/multi rotation trace. */
  function multiRotateTraceEnabled() {
    if (typeof window !== 'undefined' && window.__QE_MULTI_ROTATE_TRACE__ === false) return false;
    try {
      var q = new URLSearchParams(window.location.search);
      if (q.get('multiRotateDebug') === '0') return false;
      if (q.get('multiRotateDebug') === '1') return true;
    } catch (eMr) { /* ignore */ }
    return false;
  }

  function multiRotateTrace(stage, payload) {
    if (!multiRotateTraceEnabled()) return;
    console.log(
      '%c[MULTI-ROTATE] ' + stage,
      'color:#f7a;font-weight:bold;font-size:12px',
      payload || {}
    );
  }
  ...
  function mount(rootEl, state, api) {
    ...
    var lockComparePointerdownSeen = {};
    ...
    function buildRotateLiveRefs(sceneId, buttonId, type, memberIds) {
      if (!buttonsLayer || buttonId == null) return null;
      var idEsc = String(buttonId).replace(/"/g, '');
      var refs = {
        el: buttonsLayer.querySelector('[data-exp-stage-btn="' + idEsc + '"]'),
        gizmo: buttonsLayer.querySelector('[data-exp-gizmo][data-gizmo-id="' + idEsc + '"]'),
        members: []
      };
      var t = String(type || '').toUpperCase();
    ...
    function paintRotateLiveFromDrag(drag) {
      ...
      function paintRotateVm(el, gizmoEl, vm) {
        ...
        if (isShapeType(t) && isShapeBoxV2Active()) {
          ...
          var gm = overlaySelectionMetrics(vm, layerW, layerH);
          if (gm) {
            gizmoEl.style.left = gm.gx + '%';
            gizmoEl.style.top = gm.gy + '%';
      ...
      if (dragType === 'MULTI_SELECT' && drag.memberWorldSnapshots) {
        ...
        traceFirstGroupRotateLivePaint(drag, rotBoxes.union, layerW, layerH, rotBoxes.members);
        if (drag.liveRefs) {
          paintMultiSelectLiveFast(drag.liveRefs, rotBoxes, layerW, layerH);
          if (multiRotateTraceEnabled()) {
            var traceDegPaint = Math.round(Number(drag.pendingDeg) || 0);
            if (drag.lastRotatePaintTraceDeg !== traceDegPaint) {
              drag.lastRotatePaintTraceDeg = traceDegPaint;
              multiRotateTrace('live.multi.paint', {
                pendingDeg: traceDegPaint,
                deltaDeg: +deltaMultiPaint.toFixed(2),
                union: rotBoxes.union,
                members: rotBoxes.members
              });
            }
          }
    ...
    function flushRotateLiveFrame() {
      ...
      if (type === 'MULTI_SELECT') {
        ...
        traceFirstGroupRotateAfterLiveUpdate(
          transformDrag,
          deg,
          'applyMultiSelectRotation(rotation,live:true)'
        );
        if (multiRotateTraceEnabled()) {
          var traceDegM = Math.round(Number(deg) || 0);
          if (transformDrag.lastRotateTraceDeg !== traceDegM) {
            transformDrag.lastRotateTraceDeg = traceDegM;
            traceRotateDragStage('live.multi.model', transformDrag);
          }
        }
        ...
        traceFirstGroupRotateAfterLiveUpdate(
          transformDrag,
          deg,
          'updateOverlayGroupTransform(rotation,live:true)'
        );
        if (multiRotateTraceEnabled()) {
    ...
    function clearShapeMoveLiveStyles(moveLiveRefs) {
      ...
      moveLiveRefs.items.forEach(function (item) {
        ...
        if (item.gizmo) {
          item.gizmo.classList.remove('is-live-moving');
    ...
    function canUseOverlayMoveFastPath(sceneId, groupIds) {
      ...
      for (var i = 0; i < groupIds.length; i++) {
        var vm = getOverlayItemVm(sceneId, groupIds[i]);
        if (!vm || isOverlayEffectivelyLocked(sceneId, groupIds[i])) return false;
    ...
    function buildOverlayMoveLiveRefs(sceneId, groupIds, layerW, layerH) {
      ...
      groupIds.forEach(function (id) {
        ...
        var dom = domBoxPctFromEl(el, buttonsLayer);
        if (!dom) return;
        var boxCxPx = (dom.gx / 100) * layerW;
        var boxCyPx = (dom.gy / 100) * layerH;
        el.classList.add('is-live-moving');
        if (gizmo) gizmo.classList.add('is-live-moving');
        items.push({
          id: id,
          el: el,
          gizmo: gizmo,
    ...
    function ensureShapeMoveLiveRefs(drag) {
      ...
      var groupIds = drag.isOverlayGroup
        ? (drag.groupMemberIds || [])
    ...
    function commitShapeMoveLiveDrag(drag) {
      if (!drag || drag.pendingMoveDdx == null) return;
      if (drag.isOverlayGroup) return;
      var ddx = drag.pendingMoveDdx;
      var ddy = drag.pendingMoveDdy != null ? drag.pendingMoveDdy : 0;
      var sz = overlayLayerSize();
      if (drag.origins && drag.groupIds && drag.groupIds.length > 1) {
        drag.groupIds.forEach(function (id) {
          var orig = drag.origins[String(id)];
          if (!orig) return;
      ...
      if (drag.buttonId != null && drag.originX != null && drag.originY != null) {
        ExperienciaEngine.setSceneButtonPosition(
          ...
          drag.originX + ddx, drag.originY + ddy, sz.w, sz.h
        );
      }
    }
      ...
      moveLiveRefs.items.forEach(function (item) {
        ...
        if (item.gizmo) {
          var gTf =
            'translate3d(calc(-50% + ' + dxPx + 'px), calc(-50% + ' + dyPx + 'px), 0) ' +
            'rotate(' + rot + 'deg)';
          if (item.lastGizmoTf !== gTf) {
            item.lastGizmoTf = gTf;
            item.gizmo.style.transform = gTf;
          }
        }
      });
    ...
    function openOverlayRotationEditor(clientX, clientY, sceneId, buttonId, anchorEl) {
      ...
      if (!btn || isOverlayEffectivelyLocked(sceneId, buttonId)) return;
    ...
    function paintButtonsStage() {
      ...
      var selSet = buildOverlayVisualSelSet(canvas().selectedId, selIds);
      var editGroupId = canvas().activeOverlayGroupEditId;
      var editMemberSet = {};
      if (editGroupId) {
        overlayGroupMemberIdList(canvas().selectedId, editGroupId).forEach(function (mid) {
          editMemberSet[String(mid)] = true;
        });
      }
      var guidesHtml = '';
      ...
      if (selIds.length) {
        ...
        if (groupMount) {
          ...
          selIds.forEach(function (sid) {
            if (isOverlayEffectivelyLocked(canvas().selectedId, sid)) return;
            if (isOverlayGroupId(canvas().selectedId, sid) &&
                !canvas().activeOverlayGroupEditId) {
              return;
            }
    ...
    function mountOverlaySelectionGizmos(selIds) {
      ...
      var sceneId = canvas().selectedId;
      if (!sceneId) return;
      var gizmoIds = (selIds || []).map(String).filter(function (id) {
        return id && !isOverlayEffectivelyLocked(sceneId, id);
      });
      var layerW = overlayLayerSize().w;
      var layerH = overlayLayerSize().h;
      var selSet = buildOverlayVisualSelSet(sceneId, selIds);
      var editGroupIdMount = canvas().activeOverlayGroupEditId;
      var editMemberSetMount = {};
      if (editGroupIdMount) {
        overlayGroupMemberIdList(sceneId, editGroupIdMount).forEach(function (mid) {
          editMemberSetMount[String(mid)] = true;
        });
      }
      ...
      buttonsLayer.querySelectorAll('[data-exp-stage-btn]').forEach(function (el) {
        ...
        if (editMemberSetMount[String(id)]) el.classList.add('is-group-edit-member');
        else el.classList.remove('is-group-edit-member');
      });
      var multiSel = gizmoIds.length > 1;
      var gizmoHtml = '';
      var groupMountG = !multiSel ? resolveOverlayGroupGizmoMount(sceneId, gizmoIds) : null;
      if (groupMountG) {
        gizmoHtml = mountGroupSelectionGizmoHtml(sceneId, groupMountG, layerW, layerH);
      } else {
        gizmoIds.forEach(function (sid) {
          if (isOverlayGroupId(sceneId, sid) && !canvas().activeOverlayGroupEditId) return;
        ...
        if (multiSel) {
          var unionMount = getMultiSelectUnionFrame(sceneId, gizmoIds, layerW, layerH);
          if (unionMount) {
    ...
    function filterMultiScaleOverlayIds(sceneId, ids) {
      return (ids || []).map(String).filter(function (id) {
        ...
        var vm = getOverlayItemVm(sceneId, id);
        return !!(vm && !isOverlayEffectivelyLocked(sceneId, id) && vm.visible !== false);
      });
    }
    ...
    function clearMultiSelectUnionFrame() {
      canvas().multiSelectUnionFrame = null;
    }

    /** Groups are move-only — no union bbox / rotate / scale gizmo outside edit mode. */
    function resolveOverlayGroupGizmoMount(sceneId, selIds) {
      return null;
    }

    function overlayGroupMemberIdList(sceneId, groupId) {
      if (!sceneId || !groupId) return [];
      var n = ExperienciaEngine.getNode(state, sceneId);
      if (!n) return [];
      var g = ExperienciaEngine.getInteraction(n, groupId);
      if (!g) return [];
      return ExperienciaEngine.resolveOverlayGroupMemberIds
        ? ExperienciaEngine.resolveOverlayGroupMemberIds(n, g, { repair: true })
        : (g.memberIds || []).map(String);
    }

    /** When a group is selected, highlight all members on stage (not the group id). */
    function buildOverlayVisualSelSet(sceneId, selIds) {
      var set = {};
      var editGid = canvas().activeOverlayGroupEditId;
      (selIds || []).forEach(function (id) {
        id = String(id);
        if (editGid) {
          if (!isOverlayEffectivelyLocked(sceneId, id)) set[id] = true;
          return;
        }
        if (isOverlayGroupId(sceneId, id)) {
          overlayGroupMemberIdList(sceneId, id).forEach(function (mid) {
            if (!isOverlayEffectivelyLocked(sceneId, mid)) set[String(mid)] = true;
          });
        } else if (!isOverlayEffectivelyLocked(sceneId, id)) {
          set[id] = true;
        }
      });
      return set;
      ...
      memberIds.forEach(function (mid) {
        ...
        if (vmBefore && multiScaleTraceEnabled()) {
          ...
          multiScaleTrace('member.commit', {
            ...
            patchStretchY: patch.shapeStretchY != null ? +Number(patch.shapeStretchY).toFixed(4) : null,
    ...
    /** Debug snapshot — member snap vs composed VM for rotation traces. */
    function snapshotRotateContext(sceneId, targetId, dragType, drag, layerW, layerH) {
      var type = String(dragType || '').toUpperCase();
      var memberIds = drag && drag.memberIds ? drag.memberIds.slice() : [];
      var members = {};
      memberIds.forEach(function (mid) {
        var vm = getOverlayItemVm(sceneId, mid);
        var snap = drag && drag.memberWorldSnapshots
          ? drag.memberWorldSnapshots[String(mid)]
          : null;
        members[String(mid)] = {
          snapRot: snap ? +(Number(snap.rotation) || 0).toFixed(2) : null,
          snapX: snap ? +(Number(snap.cx) || 0).toFixed(4) : null,
          snapY: snap ? +(Number(snap.cy) || 0).toFixed(4) : null,
          vmRot: vm ? +(Number(vm.rotation) || 0).toFixed(2) : null,
          vmX: vm ? +(Number(vm.x) || 0).toFixed(4) : null,
          vmY: vm ? +(Number(vm.y) || 0).toFixed(4) : null
        };
      });
      var groupVm = (type === 'OVERLAY_GROUP' || type === 'GROUP')
        ? getOverlayItemVm(sceneId, targetId)
        : null;
      var orientedUnion = (type === 'OVERLAY_GROUP' || type === 'GROUP')
        ? getGroupUnionFrame(sceneId, targetId, layerW, layerH)
        : null;
      var storedUnion = null;
      if (type === 'MULTI_SELECT') {
        storedUnion = getStoredMultiSelectUnionFrame(sceneId, memberIds);
      } else if (type === 'OVERLAY_GROUP' || type === 'GROUP') {
        storedUnion = getStoredGroupOrientedFrame(sceneId, targetId);
      }
      return {
        type: type,
        targetId: targetId,
        startRot: drag ? +(Number(drag.startRot) || 0).toFixed(2) : 0,
        pendingDeg: drag && drag.pendingDeg != null ? +(Number(drag.pendingDeg)).toFixed(2) : null,
        union: drag ? {
          cx: +(Number(drag.startX) || 0).toFixed(4),
          cy: +(Number(drag.startY) || 0).toFixed(4),
          w: +(Number(drag.startW) || 0).toFixed(4),
          h: +(Number(drag.startH) || 0).toFixed(4),
          rot: drag.pendingDeg != null
            ? +(Number(drag.pendingDeg)).toFixed(2)
            : +(Number(drag.startRot) || 0).toFixed(2)
        } : null,
        orientedUnion: orientedUnion ? {
          cx: +(Number(orientedUnion.cx) || 0).toFixed(4),
          cy: +(Number(orientedUnion.cy) || 0).toFixed(4),
          w: +(Number(orientedUnion.w) || 0).toFixed(4),
          h: +(Number(orientedUnion.h) || 0).toFixed(4),
          rot: +(Number(orientedUnion.rot) || 0).toFixed(2)
        } : null,
        storedUnion: storedUnion,
        group: groupVm ? {
          rot: +(Number(groupVm.rotation) || 0).toFixed(2),
          x: +(Number(groupVm.x) || 0).toFixed(4),
          y: +(Number(groupVm.y) || 0).toFixed(4),
          w: +(Number(groupVm.width) || 0).toFixed(4),
          h: +(Number(groupVm.height) || 0).toFixed(4)
        } : null,
        members: members,
        layerW: layerW,
        layerH: layerH
    ...
    function traceRotateDragStage(stage, drag) {
      if (!multiRotateTraceEnabled() || !drag) return;
      multiRotateTrace(stage, snapshotRotateContext(
        drag.sceneId,
        drag.buttonId,
        drag.type,
        drag,
    ...
    function buildCompareRotatePayload(stage, drag) {
      ...
      return {
        stage: stage,
        mode: mode,
        actualDragType: drag.type != null ? drag.type : null,
        pendingDeg: drag.pendingDeg != null ? +(Number(drag.pendingDeg)).toFixed(2) : null,
        startRot: +(Number(drag.startRot) || 0).toFixed(2),
    ...
    function compactClusterRotateModelFields(drag, deg) {
      var mode = String(drag.type || '').toUpperCase();
      if (mode === 'OVERLAY_GROUP' || mode === 'GROUP') {
        return compactGroupModelFields(drag.sceneId, drag.buttonId);
      }
      var rot = deg != null ? Number(deg) : (drag.pendingDeg != null ? Number(drag.pendingDeg) : Number(drag.startRot));
      return {
        x: +(Number(drag.startX) || 0).toFixed(4),
        y: +(Number(drag.startY) || 0).toFixed(4),
        width: +(Number(drag.startW) || 0).toFixed(4),
        height: +(Number(drag.startH) || 0).toFixed(4),
        rotation: +(Number(rot) || 0).toFixed(2),
        storedX: null,
        storedY: null,
        _transformV: null
      };
    ...
    function traceFirstGroupRotatePipeline(stage, drag, extra) {
      if (!compareRotateEnabled() || !isClusterRotateDrag(drag)) return;
      var payload = {
        mode: String(drag.type || '').toUpperCase(),
        pendingDeg: drag.pendingDeg != null ? +(Number(drag.pendingDeg)).toFixed(2) : null,
        startRot: +(Number(drag.startRot) || 0).toFixed(2),
        x: null,
        y: null,
      ...
      var model = compactClusterRotateModelFields(
        drag,
        extra && extra.appliedDeg != null ? extra.appliedDeg : drag.pendingDeg
      );
    ...
    function traceFirstGroupRotateAfterLiveUpdate(drag, deg, step) {
      if (!compareRotateEnabled() || !isClusterRotateDrag(drag)) return;
      ...
      traceFirstGroupRotatePipeline('compareRotate.firstLiveUpdate', drag, {
        step: step || 'updateOverlayGroupTransform(rotation,live:true)',
        appliedDeg: +(Number(deg) || 0).toFixed(2)
      });
    }

    function traceFirstGroupRotateLivePaint(drag, unionBox, layerW, layerH, memberPaintBoxes) {
      if (!compareRotateEnabled() || !isClusterRotateDrag(drag)) return;
      ...
      traceFirstGroupRotatePipeline('compareRotate.firstLivePaint', drag, {
        ...
        render: buildFirstRotateLivePaintRender(
          drag, unionBox, memberPaintBoxes, layerW, layerH
        )
      });
    }
    ...
    function applyMultiSelectRotation(sceneId, memberIds, snap, pivotX, pivotY, deltaDeg, layerW, layerH, opts) {
      ...
      var traceMembers = multiRotateTraceEnabled() ? {} : null;
      ...
      memberIds.forEach(function (mid) {
        ...
        if (traceMembers) {
          traceMembers[String(mid)] = {
            snapRot: +(Number(s.rotation) || 0).toFixed(2),
            snapX: +(Number(s.cx) || 0).toFixed(4),
            snapY: +(Number(s.cy) || 0).toFixed(4),
            outRot: +(Number(b.rot) || 0).toFixed(2),
            outX: +(Number(b.cx) || 0).toFixed(4),
            outY: +(Number(b.cy) || 0).toFixed(4)
          };
        }
      ...
      if (traceMembers) {
        multiRotateTrace(opts.commit ? 'applyMultiSelectRotation.commit' : 'applyMultiSelectRotation.live', {
          sceneId: sceneId,
          deltaDeg: +deltaDeg.toFixed(2),
          pivotX: +(Number(pivotX) || 0).toFixed(4),
          pivotY: +(Number(pivotY) || 0).toFixed(4),
      ...
      (liveRefs.items || []).forEach(function (item) {
        ...
        if (isShapeType(item.kind)) {
          var vmPaint = {
            ...
            shapeStretchX: item.stretchX,
            shapeStretchY: item.stretchY
          };
    ...
    function flushMultiSelectResizeFrame(drag) {
      ...
      if (isGroupResize && boxes.union) {
        boxes.union.rot = Number(drag.startRot) || 0;
        boxes.union.kind = drag.type || 'OVERLAY_GROUP';
      }
      ...
      if (multiScaleTraceEnabled()) {
        var sampleId = drag.memberIds && drag.memberIds[0];
        if (sampleId && boxes.members[String(sampleId)]) {
          var sb = boxes.members[String(sampleId)];
          multiScaleTrace('member.live', {
            id: String(sampleId),
            sx: +Number(drag.pendingMultiSx).toFixed(6),
            w: +Number(sb.w).toFixed(4),
            h: +Number(sb.h).toFixed(4),
            stretchX: +Number(sb.stretchX).toFixed(4),
            stretchY: +Number(sb.stretchY).toFixed(4)
          });
        }
      }
    ...
    function exitOverlayGroupEditFromUser(opts) {
      opts = opts || {};
      return exitOverlayGroupEditMode({
        reselectGroup: false,
        clearSelection: true,
    ...
    function enterOverlayGroupEditMode(groupId, childId) {
      ...
      var sceneId = canvas().selectedId;
      if (isOverlayEffectivelyLocked(sceneId, childId)) return false;
      if (isOverlayEffectivelyLocked(sceneId, groupId)) return false;
      ...
      showOverlayGroupEditToast();
      paintButtonsStage();
      requestAnimationFrame(function () {
        if (String(canvas().activeOverlayGroupEditId || '') !== String(groupId)) return;
        if (String(canvas().selectedButtonId || '') !== String(childId)) return;
        mountOverlaySelectionGizmos([String(childId)]);
      });
      paintInspector();
      notifyOverlaySelection();
      return true;
    ...
    function exitOverlayGroupEditMode(opts) {
      ...
      if (opts.clearSelection) {
        canvas().selectedButtonIds = [];
        canvas().selectedButtonId = null;
      } else if (opts.reselectGroup !== false) {
        if (!isOverlayEffectivelyLocked(sceneId, groupId)) {
    ...
    function overlayPickFromSvgPoint(clientX, clientY, selectedSet, opts) {
      ...
      function scan(skipSel) {
        for (var i = nodes.length - 1; i >= 0; i--) {
          ...
          var vm = getOverlayItemVm(sceneId, id);
          if (!vm || isOverlayEffectivelyLocked(sceneId, id) || vm.visible === false) continue;
    ...
    function paintOverlayGroupMemberGlow(memberId) {
      ...
      if (isOverlayEffectivelyLocked(sceneId, memberId)) return;
    ...
    function previewOverlayGroupMember(memberId) {
      ...
      var sceneId = canvas().selectedId;
      if (!sceneId || isOverlayEffectivelyLocked(sceneId, memberId)) return false;
      if (!resolveGroupedOverlayHit(sceneId, memberId)) return false;
    ...
    function syncOverlayGroupCursor(clientX, clientY) {
      if (transformDrag) return;
      if (buttonDrag && buttonDrag.isOverlayGroup) {
        var distG = Math.hypot(
          (clientX != null ? clientX : buttonDrag.clientStartX) - buttonDrag.clientStartX,
          (clientY != null ? clientY : buttonDrag.clientStartY) - buttonDrag.clientStartY
        );
        setOverlayTransformCursor(
          (buttonDrag.historyPushed || distG >= OVERLAY_DRAG_THRESHOLD_PX)
            ? 'grabbing'
            : 'grab'
        );
        return;
      ...
      if (canvas().activeOverlayGroupEditId) {
        if (!buttonDrag) clearOverlayTransformCursor();
        return;
      }
      if (groupPointerGesture) {
        setOverlayTransformCursor('grab');
        return;
      }
      if (canvas().editMode !== 'buttons' && !overlayMode) {
        if (!buttonDrag) clearOverlayTransformCursor();
      ...
      var sceneIdCur = canvas().selectedId;
      if (!sceneIdCur || !buttonsLayer) {
        if (!buttonDrag) clearOverlayTransformCursor();
        return;
      }
      var hitCur = overlayPickTopmostAtPoint(clientX, clientY);
      if (!hitCur) {
        hitCur = pickOverlayStageBtnFromPoint(clientX, clientY, { forHover: true });
      }
      var bidCur = hitCur && hitCur.getAttribute('data-exp-stage-btn');
      if (bidCur && isOverlayEffectivelyLocked(sceneIdCur, bidCur)) {
        if (!buttonDrag) clearOverlayTransformCursor();
        return;
      }
      var groupedCur = bidCur ? resolveGroupedOverlayHit(sceneIdCur, bidCur) : null;
      if (groupedCur) {
        if (isOverlayEffectivelyLocked(sceneIdCur, groupedCur.childId) ||
            isOverlayEffectivelyLocked(sceneIdCur, groupedCur.groupId)) {
          if (!buttonDrag) clearOverlayTransformCursor();
          return;
        }
        setOverlayTransformCursor('grab');
    ...
    function overlayPickTopmostAtPoint(clientX, clientY) {
      if (!buttonsLayer) return null;
      var sceneId = canvas().selectedId;
      if (!sceneId) return null;
    ...
    /** Selected gizmo chrome when no stage node sits under the pointer. */
    function overlayPickSelectedGizmoOwnerAtPoint(clientX, clientY) {
      if (!buttonsLayer) return null;
      var sceneId = canvas().selectedId;
      if (!sceneId) return null;
      var selIds = getSelectedOverlayIds();
      ...
      try {
        ...
        for (var gi = 0; stack && gi < stack.length; gi++) {
          var gizmo = stack[gi].closest && stack[gi].closest('[data-exp-gizmo]');
          if (!gizmo || !buttonsLayer.contains(gizmo)) continue;
          var gid = String(gizmo.getAttribute('data-gizmo-id') || '');
          if (!gid || !selSet[gid] || isOverlayGroupId(sceneId, gid)) continue;
          if (isOverlayEffectivelyLocked(sceneId, gid)) continue;
          var idEsc = gid.replace(/"/g, '');
          var ownerEl = buttonsLayer.querySelector('[data-exp-stage-btn="' + idEsc + '"]');
          if (ownerEl) return ownerEl;
        }
      } catch (eGizmoPick) { /* ignore */ }
      return null;
    }
    ...
    function pickOverlayStageBtnFromPoint(clientX, clientY, opts) {
      ...
      /* Click/drag: topmost geometric hit (z-order) so overlap selection is flexible. */
      if (!opts.forHover) {
      ...
      if (stack && stack.length) {
        for (var si = 0; si < stack.length; si++) {
          ...
          if (isOverlayEffectivelyLocked(canvas().selectedId, bid)) continue;
          seen[bid] = true;
    ...
    /** Single editor gate — self lock OR ancestor group lock (silent; no diagnostics). */
    function isOverlayEffectivelyLocked(sceneId, itemId) {
    ...
    /** Item effective lock first; group lock after redirect (returns true = cancel interaction). */
    function blockOverlayEditInteraction(sceneId, itemId, groupId) {
      if (itemId && isOverlayEffectivelyLocked(sceneId, itemId)) return true;
      if (groupId && isOverlayEffectivelyLocked(sceneId, groupId)) return true;
      return false;
    }
    ...
    function buildOverlayDragOrigins(sceneId, ids) {
      ...
      ids.forEach(function (id) {
        if (isOverlayGroupId(sceneId, id)) {
          ...
          if (!gvm || isOverlayEffectivelyLocked(sceneId, id)) return;
          origins[String(id)] = {
        ...
        if (!b || isOverlayEffectivelyLocked(sceneId, id)) return;
        origins[String(id)] = {
    ...
    function nudgeSelectedButtons(dxPx, dyPx) {
      ...
      if (single && isOverlayGroupId(sceneId, ids[0]) &&
        ...
        if (!gNudge || isOverlayEffectivelyLocked(sceneId, ids[0])) return false;
      ...
      ids.forEach(function (bid) {
        ...
        if (!btn || isOverlayEffectivelyLocked(sceneId, bid)) return;
    ...
    function resolveCopyClusters(sceneId, ids) {
      ...
      ids.forEach(function (id) {
        ...
        var vm = getOverlayItemVm(sceneId, id);
        if (!vm || isOverlayEffectivelyLocked(sceneId, id) || vm.visible === false) return;
    ...
    function overlaysInOverlayMarquee(sceneId, marqueeData) {
      ...
      (ExperienciaEngine.listSceneButtons(state, n) || []).forEach(function (b) {
        if (!b || isOverlayEffectivelyLocked(sceneId, b.id) || b.visible === false) return;
        var ix = b._ix || b;
        var vm = ExperienciaEngine.buttonViewModel
    ...
    if (buttonsLayer) {
      ...
      function finishOverlayMarquee(ev) {
        ...
        if (om.moved) {
          var hitIds = overlaysInOverlayMarquee(om.sceneId, om).filter(function (id) {
            return !isOverlayEffectivelyLocked(om.sceneId, id);
          });
          if (om.shift) {
            var merged = getSelectedOverlayIds().slice().filter(function (id) {
              return !isOverlayEffectivelyLocked(om.sceneId, id);
            });
      ...
      function cancelOverlayGestures() {
        ...
        if (transformDrag) {
          ...
          if (transformDrag.rotateRaf) cancelAnimationFrame(transformDrag.rotateRaf);
          transformDrag.rotateRaf = null;
      ...
      function startGroupPointerGesture(ev, sceneId, groupId, childId) {
        if (blockOverlayEditInteraction(sceneId, childId, groupId)) return;
        groupPointerGesture = {
          sceneId: sceneId,
          groupId: String(groupId),
      ...
      function flushGroupPointerGestureToMove(ev) {
        ...
        if (isOverlayEffectivelyLocked(g.sceneId, g.childId) ||
            isOverlayEffectivelyLocked(g.sceneId, g.groupId)) {
          clearGroupPointerGesture();
      ...
      function finishGroupPointerGesture(ev) {
        ...
        releaseGroupPointerCapture(ev);
        unbindOverlayPointerDocs();
        if (canvas().activeOverlayGroupEditId) return;
        if (isOverlayEffectivelyLocked(g.sceneId, g.childId) ||
            isOverlayEffectivelyLocked(g.sceneId, g.groupId)) {
          return;
        }
      ...
      function onOverlayDocPointerMove(ev) {
        ...
        if (transformDrag && ev.pointerId === transformDrag.pointerId) {
          ...
          if (mode === 'rotate') {
            ...
            var startAng = transformDrag.startPointerAng != null
              ? transformDrag.startPointerAng
              : Math.atan2(transformDrag.startPy - transformDrag.startY,
                transformDrag.startPx - transformDrag.startX);
            var deg = Math.round(
              (Number(transformDrag.startRot) || 0) +
              ((ang - startAng) * 180) / Math.PI
            );
            deg = overlayRotationSnapDeg(deg, ev);
            if (deg > 360) deg = deg % 360;
            if (deg < -360) deg = -((-deg) % 360);
            scheduleRotateLiveFrame(transformDrag, deg);
            return;
          } else {
        ...
        if (buttonDrag.isOverlayGroup && ExperienciaEngine.updateOverlayGroupTransform) {
          ...
          if (buttonDrag.moveLiveRefs) {
            buttonDrag.pendingMoveDdx = ddx + snapDx;
            buttonDrag.pendingMoveDdy = ddy + snapDy;
      ...
      function beginOverlayMove(ev, bid, sceneId, btn, opts) {
        ...
        if (isOverlayEffectivelyLocked(sceneId, bid)) {
        ...
        var groupMemberIds = [];
        if (isGroupDrag && ExperienciaEngine.resolveOverlayGroupMemberIds) {
          var nMem = ExperienciaEngine.getNode(state, sceneId);
          var gMem = nMem && ExperienciaEngine.getInteraction
            ? ExperienciaEngine.getInteraction(nMem, bid)
            : null;
          if (gMem) {
            groupMemberIds = ExperienciaEngine.resolveOverlayGroupMemberIds(
              nMem, gMem, { repair: true }
            ) || [];
          }
        }
        clearPendingMove(bid);
      ...
      buttonsLayer.addEventListener('pointerdown', function (ev) {
        ...
        if (itemIdVerify && api.debugCompareLockState &&
            !lockComparePointerdownSeen[String(itemIdVerify)]) {
          lockComparePointerdownSeen[String(itemIdVerify)] = true;
          api.debugCompareLockState(itemIdVerify, 'pointerdown');
        }
        ...
        if (!preferShapeSelect && handle && handle.closest('[data-exp-gizmo]')) {
          ...
          if (gtype === 'OVERLAY_GROUP' || gtype === 'GROUP') return;
          ...
          if (gtype === 'MULTI_SELECT') {
            ...
            if (!btnG || isOverlayEffectivelyLocked(sceneIdG, gid)) return;
          }
          ...
          transformDrag = {
            ...
            startPointerAng: handleMode === 'rotate'
              ? Math.atan2(pct0.y - startY0, pct0.x - startX0)
              : null,
            startPx: pct0.x,
            startPy: pct0.y,
            startPtrX: ptr0.x,
        ...
        if (!preferShapeSelect && moveSurface && moveSurface.closest('[data-exp-gizmo]')) {
          ...
          if (moveGtype === 'MULTI_SELECT' || moveId === MULTI_SELECT_GIZMO_ID) {
            ...
            if (!btnMoveMulti || isOverlayEffectivelyLocked(sceneIdMove, moveIds[0])) return;
            ev.preventDefault();
            ev.stopPropagation();
          ...
          if (!btnMove || isOverlayEffectivelyLocked(sceneIdMove, moveId)) return;
          ev.preventDefault();
          ev.stopPropagation();
        ...
        if (!hit) {
          ...
          if (canvas().activeOverlayGroupEditId) {
            exitOverlayGroupEditFromUser({ persist: true });
            return;
          }
        ...
        if (groupedHit) {
          if (blockOverlayEditInteraction(sceneIdHit, bid, groupedHit.groupId)) return;
          var gid = groupedHit.groupId;
          var cid = groupedHit.childId;
          var inGroupEdit = String(canvas().activeOverlayGroupEditId || '') === gid;

          if (inGroupEdit) {
            if (isOverlayEffectivelyLocked(sceneIdHit, cid)) return;
            groupEditPulse = null;
            ev.preventDefault();
            ev.stopPropagation();
        ...
        ev.stopPropagation();
        var bidPicked = bid;
        if (isOverlayEffectivelyLocked(sceneIdHit, bidPicked)) return;
        bid = resolveOverlayPickId(sceneIdHit, bid);
        if (String(bid) !== String(bidPicked) && isOverlayEffectivelyLocked(sceneIdHit, bid)) return;
        ...
        if (ev.shiftKey) {
          canvas().activeOverlayGroupEditId = null;
          if (!isOverlayEffectivelyLocked(sceneIdHit, bid)) {
            var idx = cur.indexOf(String(bid));
            if (idx >= 0) cur.splice(idx, 1);
            else cur.push(String(bid));
          }
      ...
      buttonsLayer.addEventListener('dblclick', function (ev) {
        ...
        if (!grouped) return;
        if (blockOverlayEditInteraction(sceneId, grouped.childId, grouped.groupId)) return;
        ev.preventDefault();
        ev.stopPropagation();
        cancelOverlayGestures();
        groupEditPulse = null;
      ...
      function endButtonDrag(ev) {
        ...
        if (transformDrag && (!ev || ev.pointerId === transformDrag.pointerId)) {
          ...
          if (wasRotate) {
            ...
            if (!rotateFinalizeRelocalizeBlocked() &&
              ...
              if (ExperienciaEngine.relocalizeOverlayGroupMembers) {
                ...
                if (nRel && gRel) {
                  ExperienciaEngine.relocalizeOverlayGroupMembers(
                    nRel, gRel, endedDrag.layerW, endedDrag.layerH
                  );
                }
              }
              persist();
          ...
          var blockRotatePostRefresh = wasRotate && rotatePointerupPostPipelineBlocked();
        ...
        if (moved && endedMoveLiveRefs) commitShapeMoveLiveDrag(buttonDrag);
        buttonDrag = null;
        unbindOverlayPointerDocs();
        if (dragIsGroup) clearOverlayTransformCursor();
      ...
      buttonsLayer.addEventListener('pointerleave', function () {
        clearOverlayCanvasHover();
        if (!buttonDrag && !transformDrag) clearOverlayTransformCursor();
      });
      buttonsLayer.addEventListener('dblclick', function (ev) {
    ...
    return {
      ...
      exitGroupEditMode: function (opts) {
        return exitOverlayGroupEditFromUser(opts);
      },
      ...
      selectOverlayItem: function (itemId, opts) {
        ...
        if (!ix) return false;
        if (!opts.fromPanel && ExperienciaEngine.isOverlayEffectivelyLocked &&
            ExperienciaEngine.isOverlayEffectivelyLocked(state, n, ix)) {
          return false;
        }
      ...
      toggleOverlayItemSelection: function (itemId) {
        ...
        if (t === 'HOTSPOT') {
          ...
          if (idx < 0 && ExperienciaEngine.isOverlayEffectivelyLocked &&
              ExperienciaEngine.isOverlayEffectivelyLocked(state, n, ix)) {
            return false;
          }
          if (idx >= 0) cur.splice(idx, 1);
