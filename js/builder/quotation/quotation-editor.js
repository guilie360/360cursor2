...
var QuotationEditor = (function () {
  ...
  function buttonShapePickerThumbSvg(shape) {
    var kindMap = {
      square: 'SHAPE_RECT',
      rounded: 'SHAPE_ROUND_RECT',
      circle: 'SHAPE_CIRCLE',
      capsule: 'SHAPE_CAPSULE'
    };
    return shapePickerThumbSvg(kindMap[String(shape || '').toLowerCase()] || 'SHAPE_RECT');
  }
    ...
    var grid = BUTTON_SHAPE_PICKER_ITEMS.map(function (item) {
      return '' +
        '<button type="button" class="qe-shape-picker__item qe-button-picker__item" ' +
          ...
          buttonShapePickerThumbSvg(item.shape) +
        '</button>';
    }).join('');
    return '' +
  ...
  function pickMemberGroupDiagFields(ix) {
    if (!ix) return null;
    return {
      id: ix.id,
      x: ix.x,
      ...
      boxW: ix.boxW,
      boxH: ix.boxH,
      shapeStretchX: ix.shapeStretchX,
      shapeStretchY: ix.shapeStretchY,
      marginX: ix.marginX,
      marginY: ix.marginY,
      positionMode: ix.positionMode,
      anchor: ix.anchor,
      positionInitialized: ix.positionInitialized
    };
  }

  function pickGroupFrameDiagFields(g) {
    if (!g) return null;
    return {
      id: g.id,
      x: g.x,
      ...
      _transformV: g._transformV,
      _baseWidth: g._baseWidth,
      _baseHeight: g._baseHeight
    };
  }

  /** Data-only — shim SSOT + overlayWorldLayoutRaw (same path as buttonViewModel / compose). */
  function readShimMemberGroupDiag(groupId, memberId, lw, lh) {
    if (!expOverlay || !expOverlay.shim || typeof ExperienciaEngine === 'undefined') return null;
    var n = ExperienciaEngine.getNode(expOverlay.shim, overlayPanelNodeId());
    if (!n) return null;
    var ix = ExperienciaEngine.getInteraction(n, memberId);
    var grp = ExperienciaEngine.getInteraction(n, groupId);
    if (!ix || !grp) return null;
    var computedWorld = ExperienciaEngine.overlayWorldLayoutRaw
      ? ExperienciaEngine.overlayWorldLayoutRaw(n, ix, lw, lh)
      : null;
    var rendererVm = ExperienciaEngine.buttonViewModel
      ? ExperienciaEngine.buttonViewModel(expOverlay.shim, n, ix, lw, lh)
      : null;
    return {
      stored: pickMemberGroupDiagFields(ix),
      group: pickGroupFrameDiagFields(grp),
      computedWorld: computedWorld ? {
        x: computedWorld.x,
        y: computedWorld.y,
        rotation: computedWorld.rotation,
        width: computedWorld.width,
        height: computedWorld.height,
        boxW: computedWorld.boxW,
        boxH: computedWorld.boxH
      } : null,
      rendererVm: rendererVm ? {
        x: rendererVm.x,
        y: rendererVm.y,
        rotation: rendererVm.rotation,
        storedX: rendererVm.storedX,
        storedY: rendererVm.storedY
      } : null
    };
  }

  function logAGroupPositionPhase(phase, groupId, memberId, lw, lh) {
    if (!groupId || !memberId) return;
    var snap = readShimMemberGroupDiag(groupId, memberId, lw, lh);
    if (!snap) return;
    var diag = window.__QE_A_GROUP_DIAG__;
    if (!diag) diag = window.__QE_A_GROUP_DIAG__ = { memberId: null, groupId: null, logged: {} };
    if (diag.logged && diag.logged[phase]) return;

    if (phase === 'A-after-first-group') {
      diag.memberId = String(memberId);
      diag.groupId = String(groupId);
      diag.logged = {};
    } else if (phase === 'before-B' || phase === 'after-B') {
      if (!diag.memberId || String(memberId) !== String(diag.memberId)) return;
      if (String(groupId) !== String(diag.groupId)) return;
    }

    diag.logged[phase] = true;
    console.log(
      '[A GROUP POSITION]\n' +
      'phase=' + phase + '\n' +
      'stored=' + JSON.stringify(snap.stored) + '\n' +
      'group=' + JSON.stringify(snap.group) + '\n' +
      'computedWorld=' + JSON.stringify(snap.computedWorld) + '\n' +
      'rendererVm=' + JSON.stringify(snap.rendererVm)
    );
  }
  ...
  function overlayPanelNodeId() {
    var sceneId = state.backpackMode ? BACKPACK_SCENE_ID : state.activeSceneId;
    return (typeof QuotationExperienciaBridge !== 'undefined' &&
      QuotationExperienciaBridge.nodeIdForScene)
      ? QuotationExperienciaBridge.nodeIdForScene(sceneId)
      : ('qe-' + sceneId);
  }
  ...
  function copyPanelInteractionFromShim(interactionId) {
    ...
    var panelIx = findSceneInteraction(interactionId);
    if (!panelIx) return;
    var n = ExperienciaEngine.getNode(expOverlay.shim, overlayPanelNodeId());
    if (!n) return;
    var shimIx = ExperienciaEngine.getInteraction(n, interactionId);
    if (!shimIx) return;
    if (shimIx.groupId) {
      panelIx.groupId = shimIx.groupId;
      panelIx.localX = shimIx.localX;
      panelIx.localY = shimIx.localY;
      panelIx.localRotation = shimIx.localRotation;
    } else {
      delete panelIx.groupId;
      delete panelIx.localX;
      delete panelIx.localY;
      delete panelIx.localRotation;
      panelIx.x = shimIx.x;
      panelIx.y = shimIx.y;
      panelIx.rotation = shimIx.rotation;
      if (shimIx.width != null) panelIx.width = shimIx.width;
      if (shimIx.height != null) panelIx.height = shimIx.height;
      if (shimIx.boxW != null) panelIx.boxW = shimIx.boxW;
      if (shimIx.boxH != null) panelIx.boxH = shimIx.boxH;
    }
  }
  ...
  function assignInteractionToGroup(memberId, groupId, beforeMemberId) {
    ...
    if (existingBeforeJoin.length) {
      watchMemberId = String(existingBeforeJoin[0]);
      logAGroupPositionPhase('before-B', groupId, watchMemberId, lw, lh);
    }
    ...
    healOverlayGroupMembership(scene);
    if (!existingBeforeJoin.length) {
      logAGroupPositionPhase('A-after-first-group', groupId, memberId, lw, lh);
    } else if (watchMemberId) {
      logAGroupPositionPhase('after-B', groupId, watchMemberId, lw, lh);
    }
    markDirtyLocal();
    return true;
  }
  ...
  function reorderGroupMembers(groupId, dragId, targetId, position) {
    ...
    if (beforeKey === ids.join('|')) return false;
  ...
  function outlinerListRowEl(list, id) {
    if (!list || id == null) return null;
    var rows = list.querySelectorAll('[data-qe-outliner-row]');
    var want = String(id);
    var i;
    for (i = 0; i < rows.length; i++) {
      if (String(rows[i].getAttribute('data-qe-outliner-row') || '') === want) {
        return rows[i];
      }
    }
    return null;
  ...
  function bindOutlinerDnD(body) {
    ...
    function clearDropMarkersVisual() {
      var list = listEl();
      if (!list) return;
    ...
    function rowDropAt(clientY, dragId) {
      ...
      for (i = 0; i < rows.length; i++) {
        ...
        if (!rid || String(rid) === String(dragId)) continue;
        var h = rect.height || 1;
    ...
    function paintDropMarker(target) {
      ...
      if (target && String(target.id) !== String(draggingId)) {
        dropTarget = target;
        var row = outlinerListRowEl(list, target.id);
        if (row) {
      ...
      var dragRow = outlinerListRowEl(list, draggingId);
    ...
    body.addEventListener('pointerdown', function (e) {
      ...
      paintDropMarker(rowDropAt(e.clientY, draggingId));
    }, true);
  }
  ...
  function clearPendingAddButtonAction() {
    if (pendingExpAction && pendingExpAction.type === 'addButton') {
  ...
  function closeButtonPicker() {
    var pickerVisible = !!(rootEl && rootEl.querySelector('[data-qe-button-picker]'));
    if (!state.buttonPickerOpen && !pickerVisible) return;
    state.buttonPickerOpen = false;
    clearPendingAddButtonAction();
    rerender();
  }
